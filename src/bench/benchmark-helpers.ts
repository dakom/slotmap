...
export const prep_mock_data = ():[MySlotMap, MyNativeMap] => {
    ...
    for(let i = 0; i < N_ENTITIES; i++) {
        slotmap_keys.push(insert_slotmap(slotmap));
        nativemap_keys.push(insert_nativemap(nativemap));
    }

    shuffle(slotmap_keys);
    shuffle(nativemap_keys);

    for(let i = 0; i < 5; i++) {
        const tmp_slotmap_keys = slotmap_keys.splice(0,N_ENTITIES/2);
        const tmp_nativemap_keys = nativemap_keys.splice(0,N_ENTITIES/2);

        tmp_slotmap_keys.forEach(key => {
            slotmap.remove(key);
        });
        tmp_nativemap_keys.forEach(key => {
